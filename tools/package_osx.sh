...
########################################################################################################################
echo "Configure with Python ${PYTHON_VER}..." && echo -en "${TRAVIS_BEG}"
cmake $SOURCE_PATH -Bbuild_${PYTHON_VER_SUFFIX} -DPYTHON_EXECUTABLE=$PYTHON_DIR/bin/python -DPACKAGE_OUTPUT_FILE_PREFIX=${ARTIFACT_PATH} -DENABLE_PORTABLE=ON -DPACKAGE_SUFFIX=py${PYTHON_VER_SUFFIX} ${CMAKE_EXTRA_FLAGS}
echo -en "${TRAVIS_END}"

echo "Test with Python ${PYTHON_VER}..." && echo -en "${TRAVIS_BEG}"
cmake --build build_${PYTHON_VER_SUFFIX} --target check -- -j4
echo -en "${TRAVIS_END}"
